...
class ImProcessing:
    ...
    def extractData(self, img_path: str, ncoins=1) -> dict[str, list]:
        ...
        # Normalizamos
        images = [self.clahe(cv2.cvtColor(img, cv2.COLOR_GRAY2BGR))
                  for img in images]
    ...
    def removeExternalRing(img: cv2.Mat) -> cv2.Mat:
    ...
    def getLines(imgs: list[cv2.Mat]):
    ...
    def getAreas(self, img: cv2.Mat):
        ...
        # Ahora eliminamos el anillo exterior de la moneda
        edges = self.removeExternalRing(edges)
